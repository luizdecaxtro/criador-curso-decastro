  ...
  async function moduleWithRetry(m) {
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        if (attempt === 0) await new Promise(r => setTimeout(r, 1500 + Math.floor(Math.random() * 1500)));
      }
    }
    return null; // falhou nas duas tentativas — o módulo fica vazio, sem derrubar os demais
  }
  // gera em lotes de 2, com pausa entre eles, para respeitar o limite de taxa da API
  const modules = input.program.modules;
  const results = [];
  for (let i = 0; i < modules.length; i += 2) {
    if (i > 0) await new Promise(r => setTimeout(r, 1200)); // pausa entre lotes
    const batch = modules.slice(i, i + 2).map(m => moduleWithRetry(m));
...
// migração leve: garante as colunas de crédito (idempotente; roda 1x por isolate)
let _creditSchemaReady = false;
async function ensureCreditSchema(env) {
  if (_creditSchemaReady) return;
  try { await env.DB.prepare('ALTER TABLE creators ADD COLUMN course_credits INTEGER DEFAULT 0').run(); } catch (e) {}
  try { await env.DB.prepare('ALTER TABLE courses ADD COLUMN revisions_left INTEGER DEFAULT 3').run(); } catch (e) {}
  _creditSchemaReady = true;
}
...
async function me(env, request) {
  ...
  const creator = await env.DB.prepare('SELECT name, email, subscriber_active, subscription_expires_at, is_admin, course_credits FROM creators WHERE id = ?').bind(creatorId).first();
  ...
  return {
    ...
    courseCredits: creator.course_credits || 0,
    creditPriceCents: await getCreditPriceCents(env)
  };
}
...
async function handleInfinitePayWebhook(env, payload) {
  ...
  if (orderNsu.startsWith('credit:')) {
    // compra de créditos de criação de curso (order_nsu = credit:<creatorId>:<qty>)
    const [, creatorId, qtyStr] = orderNsu.split(':');
    if (!creatorId) return { ok: false, reason: 'order_nsu de crédito malformado' };
    const qty = Math.max(1, parseInt(qtyStr) || 1);
    await ensureCreditSchema(env);
    await env.DB.prepare('UPDATE creators SET course_credits = COALESCE(course_credits,0) + ? WHERE id = ?').bind(qty, creatorId).run();
    return { ok: true };
  }
...
async function getCreditPriceCents(env) {
  try {
    const row = await env.DB.prepare("SELECT value FROM settings WHERE key = 'credit_price_cents'").first();
    if (row && parseInt(row.value) > 0) return parseInt(row.value);
  } catch (e) {}
  return parseInt(env.CREDIT_PRICE_CENTS) || 9000;
}

async function adminGetPricing(env, request) {
  await requireAdmin(env, request);
  const p = await getPricingInfo(env);
  return { priceCents: p.priceCents, fullPriceCents: p.fullPriceCents, creditPriceCents: await getCreditPriceCents(env) };
}

async function adminSavePricing(env, request, body) {
  await requireAdmin(env, request);
  await env.DB.prepare('CREATE TABLE IF NOT EXISTS settings (key TEXT PRIMARY KEY, value TEXT)').run();
  const setKey = async (key, cents) => {
    await env.DB.prepare("INSERT INTO settings (key, value) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET value=excluded.value").bind(key, String(cents)).run();
  };
  if (body.priceCents !== undefined && body.fullPriceCents !== undefined) {
    ...
    await setKey('subscription_price_cents', priceCents);
    await setKey('subscription_full_price_cents', fullPriceCents);
  }
  if (body.creditPriceCents !== undefined) {
    const cp = parseInt(body.creditPriceCents);
    if (!Number.isFinite(cp) || cp <= 0) throw new Error('Preço do crédito inválido');
    await setKey('credit_price_cents', cp);
  }
  return await adminGetPricing(env, request);
}

async function buyCreditsCheckout(env, request, qty) {
  const creatorId = await getCreatorId(env, request);
  if (!creatorId) throw new Error('É preciso estar logado');
  qty = Math.max(1, Math.min(20, parseInt(qty) || 1));
  const priceCents = await getCreditPriceCents(env);
  const payload = {
    handle: CHECKOUT_HANDLE,
    items: [{ quantity: qty, price: priceCents, description: 'Crédito para criação de curso' }],
    order_nsu: 'credit:' + creatorId + ':' + qty,
  ...
  const url = data && (data.url || data.checkout_url || data.link || data.payment_url);
  if (!url) throw new Error('InfinitePay respondeu sem um link de checkout reconhecível: ' + raw.slice(0, 300));
  return { url };
}
...
async function deleteCourse(env, request, courseId) {
  const creatorId = await getCreatorId(env, request);
  if (!creatorId) throw new Error('É preciso estar logado');
  if (!courseId) throw new Error('Parâmetro courseId é obrigatório');
  const course = await env.DB.prepare('SELECT creator_id, price_cents FROM courses WHERE id = ?').bind(courseId).first();
  if (!course) throw new Error('Curso não encontrado');
  if (course.creator_id !== creatorId) throw new Error('Você só pode excluir os seus próprios cursos');
  // protege cursos pagos que já tenham aluno pagante inscrito
  if (course.price_cents && course.price_cents > 0) {
    const paying = await env.DB.prepare('SELECT COUNT(*) AS cnt FROM enrollments WHERE course_id = ? AND paid = 1').bind(courseId).first();
    if (paying && paying.cnt > 0) {
      throw new Error(`Este curso tem ${paying.cnt} aluno(s) pagante(s) inscrito(s). Não é possível excluí-lo.`);
    }
  }
  await env.DB.prepare('DELETE FROM lessons WHERE module_id IN (SELECT id FROM modules WHERE course_id = ?)').bind(courseId).run();
  await env.DB.prepare('DELETE FROM modules WHERE course_id = ?').bind(courseId).run();
  await env.DB.prepare('DELETE FROM enrollments WHERE course_id = ?').bind(courseId).run();
  await env.DB.prepare('DELETE FROM guest_sessions WHERE course_id = ?').bind(courseId).run();
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (request.method === 'POST' && url.pathname === '/api/buy-credits') {
        const body = await request.json();
        const result = await buyCreditsCheckout(env, request, body.qty);
        return jsonResponse(result, 200, headersOut, cors);
      }
      ...
      if (request.method === 'POST' && url.pathname === '/api/delete-course') {
        const body = await request.json();
        const result = await deleteCourse(env, request, body.courseId);
        return jsonResponse(result, 200, headersOut, cors);
      }
