...
Mantenha os títulos de aula EXATAMENTE como foram informados.`;
  ...
  const maxTokens = Math.min(16000, 3000 + (module.lessons ? module.lessons.length : 3) * 1400);
...
// A caligrafia (Great Vibes) é buscada do Google Fonts para dar o toque de assinatura.
...
const FONT_URLS = {
  script: 'https://raw.githubusercontent.com/google/fonts/main/ofl/greatvibes/GreatVibes-Regular.ttf'
};
...
async function generateApostilaPdf(env, request, courseId) {
  ...
  function activityBox(title, body) {
    ...
    const bodyLines = [];
    (String(body || '').split(/\n\s*\n/)).forEach(par => {
      const words = safe(par.replace(/\s+/g, ' ').trim(), serif).split(/\s+/).filter(Boolean);
      let line = '';
      for (const w of words) { const test = line ? line + ' ' + w : w; if (serif.widthOfTextAtSize(test, 11.5) > CW - 28) { if (line) bodyLines.push(line); line = w; } else line = test; }
      if (line) bodyLines.push(line);
      bodyLines.push(''); // espaço entre parágrafos
    });
    while (bodyLines.length && bodyLines[bodyLines.length - 1] === '') bodyLines.pop();
    const titleH = 16, pad = 12, lineH = 16;
    const boxH = pad + titleH + 6 + bodyLines.length * lineH + pad;
    ensure(boxH + 12);
    cur.y -= 12;
    ...
    let yy = cur.y - pad;
    cur.page.drawText(safe(title, serifBold), { x: ML + 14, y: yy - titleH + 3, size: 12.5, font: serifBold, color: navy });
    yy -= titleH + 6;
    bodyLines.forEach(ln => { if (ln) cur.page.drawText(ln, { x: ML + 14, y: yy - 11.5, size: 11.5, font: serif, color: ink }); yy -= lineH; });
    cur.y -= boxH + 8;
  }
