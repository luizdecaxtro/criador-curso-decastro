...
// Só o front hospedado no GitHub Pages pode falar com este Worker.
// Adicione outros domínios aqui se o app passar a ser servido de mais de um lugar
// (ex: um domínio próprio no lugar do github.io).
const ALLOWED_ORIGINS = [
  'https://luizdecaxtro.github.io'
];
...
function corsHeaders(request) {
  ...
  // Se a origem não estiver na whitelist, nenhum header Access-Control-Allow-Origin
  // é devolvido — o navegador do lado do cliente bloqueia a leitura da resposta.
  return headers;
}
...
async function saveCourse(env, request, body, headersOut) {
  ...
  // Se o curso já existe, só o dono (ou, para curso ainda "de convidado", o próprio
  // fluxo de guest_sessions) pode sobrescrevê-lo.
  const existing = await env.DB.prepare('SELECT creator_id FROM courses WHERE id = ?').bind(courseId).first();
  if (existing && existing.creator_id && existing.creator_id !== creatorId) {
...
// Só o criador do curso, ou um aluno matriculado nele, pode acessar o conteúdo completo.
async function assertCanAccessCourse(env, creatorId, courseId, courseCreatorId) {
  if (creatorId && creatorId === courseCreatorId) return;
  if (creatorId) {
    const enrollment = await env.DB.prepare('SELECT id FROM enrollments WHERE student_id = ? AND course_id = ?')
      .bind(creatorId, courseId).first();
    if (enrollment) return;
  }
  throw new Error('Você não tem permissão para acessar este curso');
}

async function loadCourseForOwnerOrStudent(env, request, courseId) {
  const requesterId = await getCreatorId(env, request);
  const data = await loadCourse(env, courseId);
  await assertCanAccessCourse(env, requesterId, courseId, data.creatorId);
  return data;
}
...
async function getCourseForStudent(env, request, courseId) {
  ...
  const courseData = await loadCourse(env, courseId);
...
async function generateApostilaPdf(env, request, courseId) {
  const data = await loadCourseForOwnerOrStudent(env, request, courseId);
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (request.method === 'GET' && url.pathname === '/api/load-course') {
        ...
        const result = await loadCourseForOwnerOrStudent(env, request, courseId);
        return jsonResponse(result, 200, headersOut, cors);
      }
