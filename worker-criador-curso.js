...
async function handleInfinitePayWebhook(env, payload) {
  ...
  // order_nsu foi definido como o id do professor (creators.id) no momento da criação do checkout
  await env.DB.prepare('UPDATE creators SET subscriber_active = 1 WHERE id = ?').bind(orderNsu).run();
  return { ok: true };
}
...
async function createCheckout(env, request) {
  ...
  // (5400 = R$ 54,00, o promocional; troque para 18000 = R$ 180,00 quando o teste terminar)
...
async function listMyCourses(env, request) {
  ...
  const rows = await env.DB.prepare(
    'SELECT id, title, hours, level, updated_at FROM courses WHERE creator_id = ? ORDER BY updated_at DESC'
  ).bind(creatorId).all();
